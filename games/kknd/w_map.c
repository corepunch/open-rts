...
static bool decode_mapd(const uint8_t *segment, size_t size, uint32_t mapd_offset,
    ...
    uint32_t layer_offsets[MAX_LAYERS] = {0};
...
bool load_kknd_map(const char *map_path, level_t *out) {
    ...
    if (!open_lvl(map_path, &blob, &segment, &segment_size)) return false;
    uint32_t mapd = 0;
    if (!lvl_asset(segment, segment_size, "MAPD", 0, &mapd)) {
        fprintf(stderr, "%s has no MAPD asset\n", map_path);
        W_FreeFile(&blob);
        return false;
    }
    KkndMapData *native = calloc(1, sizeof(*native));
    if (!native || !decode_mapd(segment, segment_size, mapd, native)) {
        fprintf(stderr, "%s has an invalid or unsupported KKnD MAPD asset\n", map_path);
        map_data_destroy(native);
        W_FreeFile(&blob);
        return false;
    }
    W_FreeFile(&blob);
