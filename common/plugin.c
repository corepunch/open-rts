...
static const Plugin *loaded_plugins[MAX_LOADED_PLUGINS];
...
bool plugin_load(const char *so_path) {
    ...
    plugin_entry_fn fn = (plugin_entry_fn)dlsym(handle, "open_rts_plugin_entry");
    if (!fn) {
...
const GameDefinition *plugin_definition(const Plugin *plugin) {
    return plugin ? &plugin->definition : NULL;
}

const GameLoaders *plugin_loaders(const Plugin *plugin) {
    return plugin ? &plugin->loaders : NULL;
}
