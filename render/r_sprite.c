...
/* One upload surface belongs to the active renderer, never to an image/team.
...
static SDL_Texture *sprite_upload(SDL_Renderer *renderer, const spritesheet_t *sprite,
                                   int frame, int palette, irect_t rect) {
    ...
    if (SDL_LockTexture(sprite_buffer.texture, &area, &pixels, &pitch) != 0)
        return NULL;
    int stride = sprite->cells[frame].rect.w;
    for (int y = 0; y < rect.h; ++y) {
        uint32_t *row = (uint32_t *)((uint8_t *)pixels + (size_t)y * pitch);
        const uint8_t *source = sprite->lumps[frame].indices +
...
bool R_DrawSprite(SDL_Renderer *renderer, const spritesheet_t *sprite, int frame,
    ...
    irect_t rect = src ? *src : sprite->cells[frame].rect;
    irect_t cell = sprite->cells[frame].rect;
    if (rect.x < 0 || rect.y < 0 || rect.w <= 0 || rect.h <= 0 ||
        rect.w > cell.w || rect.h > cell.h || rect.x > cell.w - rect.w ||
        rect.y > cell.h - rect.h) return false;
    SDL_Texture *texture = sprite->lumps[frame].texture;
    if (sprite->lumps[frame].indices) {
        texture = sprite_upload(renderer, sprite, frame, palette, rect);
        rect.x = rect.y = 0;
    }
    ...
    bool ok = SDL_RenderCopyEx(renderer, texture, &rect, dst, 0, NULL, flip) == 0;
