...
static bool info_mkdir(const char *path) {
    char parent[1024];
...
static bool write_info_inc(const char *directory, const info_entry_t *entry) {
    char path[1024];
    if (!info_mkdir(directory)) return false;
    snprintf(path, sizeof(path), "%s/%s.inc", directory, entry->sprite);

    FILE *file = fopen(path, "w");
    if (!file) return false;
    fprintf(file, "/* Generated by tools/7legion_info_gen.c. Do not edit. */\n");
    fprintf(file, "    [S_%s_STND] = { SPR_%s, 0, %d, %s, S_%s_STND, 0 },\n",
            entry->sprite, entry->sprite, entry->damage ? 1 : -1,
            entry->damage ? "A_Look" : "NULL", entry->sprite);
    if (entry->damage)
        fprintf(file, "    [S_%s_FIRE] = { SPR_%s, 0, 1, A_Attack, S_%s_STND, 3 },\n",
                entry->sprite, entry->sprite, entry->sprite);
...
static bool write_info_c(const char *path, const char *source, const char *extra_include,
    ...
    fprintf(file, "};\n\nconst state_t states[NUMSTATES] = {\n"
                  "    { 0, 0, -1, NULL, S_NULL, 0 },\n");
    for (int i = 0; i < count; ++i) {
        fprintf(file, "    #include \"animate/%s.inc\"\n", entries[i].sprite);
    }
