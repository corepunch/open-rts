...
static bool write_sprite_inc(const char *directory, const unit_t *u,
                             const anim_t *a) {
    char path[1024];
    snprintf(path, sizeof(path), "%s/%s.inc", directory, u->spr_suffix);
    FILE *f = fopen(path, "w");
    if (!f) return false;

    fprintf(f, "/* Generated by tools/kknd_info_gen.c. Do not edit. */\n");
    fprintf(f, "    [S_%s_STND] = { %s, %d, %d, %s, S_%s_STND, 0 },\n",
            u->spr_suffix, u->spr_name, a->idle,
            (strcmp(u->spr_suffix, "SURV_RESEARCH_LAB") == 0 ||
             strcmp(u->spr_suffix, "MUTE_ALCHEMY_HALL") == 0) ? 1 : 5,
            (strcmp(u->spr_suffix, "SURV_RESEARCH_LAB") == 0 ||
             strcmp(u->spr_suffix, "MUTE_ALCHEMY_HALL") == 0) ? "A_KkndResearch" : "A_Chase",
            u->spr_suffix);

    for (int k = 1; a->move >= 0 && k <= a->mlen; ++k) {
        int next = k < a->mlen ? k + 1 : 1;
        fprintf(f, "    [S_%s_WALK%d] = { %s, %d, 4, NULL, S_%s_WALK%d, 2 },\n",
                u->spr_suffix, k, u->spr_name, a->move + k - 1,
                u->spr_suffix, next);
    }

    for (int k = 1; a->shoot >= 0 && k <= a->slen; ++k) {
        bool flash = a->muzzle && k == 1;
        int frame = flash ? a->move + a->mlen : a->shoot + k - 1;
        int tics = flash ? 2 : 4;
        const char *next;
        char next_name[80];
        if (flash)
            snprintf(next_name, sizeof(next_name), "S_%s_FLASH2", u->spr_suffix);
        else if (k < a->slen)
            snprintf(next_name, sizeof(next_name), "S_%s_ATCK%d", u->spr_suffix, k + 1);
        else
            snprintf(next_name, sizeof(next_name), "%s", u->state_name);
        next = next_name;
        fprintf(f, "    [S_%s_ATCK%d] = { %s, %d, %d, %s, %s, 3 },\n",
                u->spr_suffix, k, u->spr_name, frame, tics,
                k == 1 ? "A_Attack" : "NULL", next);
    }
    if (a->shoot < 0 && u->is_combat) {
        char next[80];
        if (a->muzzle) snprintf(next, sizeof(next), "S_%s_FLASH2", u->spr_suffix);
        else snprintf(next, sizeof(next), "%s", u->state_name);
        fprintf(f, "    [S_%s_ATCK1] = { %s, %d, %d, A_Attack, %s, 3 },\n",
                u->spr_suffix, u->spr_name, a->muzzle ? a->move + a->mlen : a->idle,
                a->muzzle ? 2 : 4, next);
    }
    if (a->muzzle) {
        char next[80];
        if (a->slen > 1) snprintf(next, sizeof(next), "S_%s_ATCK2", u->spr_suffix);
        else snprintf(next, sizeof(next), "%s", u->state_name);
        fprintf(f, "    [S_%s_FLASH2] = { %s, %d, 2, NULL, %s, 3 },\n",
                u->spr_suffix, u->spr_name, a->move + a->mlen + 1, next);
    }
    return fclose(f) == 0;
}

static bool write_extra_inc(const char *directory) {
    char path[1024];
    snprintf(path, sizeof(path), "%s/EXTRAS.inc", directory);
    FILE *f = fopen(path, "w");
    if (!f) return false;
    fprintf(f, "/* Generated by tools/kknd_info_gen.c. Do not edit. */\n");
    for (size_t i = 0; i < sizeof(DEATHS) / sizeof(*DEATHS); ++i) {
...
static void write_info_c(const char *path) {
    ...
    fprintf(f, "    #include \"animate/EXTRAS.inc\"\n");
    fprintf(f, "};\n\n");
...
int main(int argc, char **argv) {
    if (argc != 5) {
        fprintf(stderr,
            "kknd_info_gen: usage: kknd_info_gen <UNITS.CFG> <info.h> <info.c> <animate-dir>\n"
    ...
    if (!mkdir_p(animate_dir)) {
        fprintf(stderr, "kknd_info_gen: cannot create %s: %s\n", animate_dir, strerror(errno));
        return 1;
    }
    for (int i = 0; i < g_unit_count; ++i) {
        if (!write_sprite_inc(animate_dir, &g_units[i], &ANIM_TABLE[i])) {
            fprintf(stderr, "kknd_info_gen: cannot write %s/%s.inc: %s\n",
                    animate_dir, g_units[i].spr_suffix, strerror(errno));
            return 1;
        }
    }
    if (!write_extra_inc(animate_dir)) {
        fprintf(stderr, "kknd_info_gen: cannot write %s/EXTRAS.inc: %s\n",
                animate_dir, strerror(errno));
        return 1;
    }
