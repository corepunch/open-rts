...
/* One source for enum IDs, state rows, and exact native label references. */
static void blood_states(const char *root) {
    char path[1024];
    snprintf(path, sizeof(path), "%s/ANIMATE", root);
    struct dirent **entries;
    int count = scandir(path, &entries, NULL, alphasort);
    if (count < 0) die(path, strerror(errno));
    puts("/* Generated by dc_info_conv --blood-states. FIN layers remain in the assets. */");
    for (int i = 0; i < count; ++i) {
        char *name = entries[i]->d_name;
        char *dot = strrchr(name, '.');
        if (!dot || strcasecmp(dot, ".FIN")) { free(entries[i]); continue; }
        snprintf(path, sizeof(path), "%s/ANIMATE/%s", root, name);
        fin_t fin;
        if (!load_fin(path, &fin)) {
            fprintf(stderr, "dc_info_conv: skipping unsupported FIN %s\n", path);
            free_fin(&fin); free(entries[i]); continue;
        }
        *dot = 0;
        unsigned cells = 0;
        snprintf(path, sizeof(path), "%s/SPRITES/%s.SPR", root, name);
        FILE *spr = fopen(path, "rb");
        if (spr) {
            unsigned char header[4];
            if (fread(header, 1, 4, spr) != 4) die(path, "missing SPR header");
            cells = word(header + 2);
            fclose(spr);
        }
        for (unsigned l = 0; l < fin.label_count; ++l) {
            const unsigned char *label = fin.labels + l * 20;
            char symbol[17]; snprintf(symbol, sizeof(symbol), "%.16s", label);
            if (!strstr(symbol, "BLOOD")) continue;
            unsigned first = word(label + 16), last = word(label + 18);
            printf("DC_BLOOD_LABEL(\"%s\", S_%s_%u)\n", symbol, symbol, first);
            unsigned native_total = 0, elapsed = 0;
            for (unsigned f = first; f <= last; ++f) {
                if (f == first + 1) native_total = elapsed = 0;
                unsigned raw = word(fin.frames + f * 164 + 2);
                unsigned ticks = ((raw ? raw : 15) + 3) * 15 / 100;
                /* 0x423dd0 consumes the low byte; zero underflows to 256 ticks. */
                ticks = (uint8_t)ticks ? (uint8_t)ticks : 256;
                /* Retail default tick interval is 66 ms (+0x970). Round
                 * cumulative boundaries to the engine's 30 Hz clock. */
                native_total += ticks;
                unsigned boundary = (native_total * 66 * 30 + 500) / 1000;
                printf("DC_BLOOD_STATE(S_%s_%u, SPR_%s, %u, %u, ", symbol, f, name, cells + f, boundary - elapsed);
                elapsed = boundary;
                if (f == last) puts("S_NULL)");
                else printf("S_%s_%u)\n", symbol, f + 1);
            }
        }
        free_fin(&fin); free(entries[i]);
    }
    free(entries);
}
...
int main(int argc, char **argv) {
    if (argc == 2 && !strcmp(argv[1], "--help")) {
        puts("dc_info_conv [--labels | --label NAME | --frame N | --cell N] FILE\n"
             ...
             "dc_info_conv --blood-states ROOT   Generate native damage state references.");
        return 0;
    }
    if (argc > 2 && !strcmp(argv[1], "--states")) return export_states(argc - 1, argv + 1);
    if (argc == 3 && !strcmp(argv[1], "--blood-states")) { blood_states(argv[2]); return 0; }
