...
def generate():
    ...
    for filename, declaration, entries in (
        ('p_blood.c', 'static const struct { const char *name; int state; } blood_labels[]', blood),
        ('info.c', 'const dc_building_sequence_t dc_building_sequences', building),
    ):
        path = GAME / filename
        source = outputs.get(path, path.read_text())
        start = source.index(' = {', source.index(declaration)) + len(' = {')
        end = source.index('\n};', start)
        outputs[path] = source[:start] + '\n' + ''.join(entries).rstrip('\n') + source[end:]
    return outputs
