...
static int assert_dark_reign(RtsGameModel *model) {
    ...
    if (snapshot_count_units_with_owner_and_type(&snapshot, 0, MT_FG_CONSTRUCTION_CREW) != 3 ||
        snapshot_count_units_with_owner_and_type(&snapshot, 1, MT_FG_CONSTRUCTION_CREW) != 3) {
    ...
    if (snapshot.unit_count != units_before_hq + 1 ||
        snapshot_count_units_with_owner_and_type(&snapshot, 0, MT_FG_HQ1) != 1 ||
    ...
    int rigs_before = snapshot_count_units_with_owner_and_type(
        &snapshot, 0, MT_FG_CONSTRUCTION_CREW);
    ...
    if (snapshot.unit_count != units_before_rig + 1 ||
        snapshot_count_units_with_owner_and_type(&snapshot, 0, MT_FG_CONSTRUCTION_CREW) != rigs_before + 1 ||
...
static int assert_dark_reign_fixed_missions(RtsGameModel *model) {
    ...
    if (snapshot.map_width != 71 || snapshot.map_height != 71 ||
        ...
        snapshot_count_units_with_owner_and_type(
            &snapshot, 0, MT_FG_CONSTRUCTION_CREW) != 3) {
